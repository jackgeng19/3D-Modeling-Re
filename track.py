...
class HMR2_4dhuman(PHALP):
    ...
    def get_detections(self, image, frame_name, t_, additional_data=None, measurments=None):
        ...
        # Define the file path for saving bounding box data
        bbox_file_path = "pred_bbox_padded.txt"

        # Convert the bounding box data to a string format
        bbox_data_str = np.array2string(pred_bbox, separator=', ')

        # Append the bounding box data to the file
        with open(bbox_file_path, "a") as file:
            file.write(f"Frame {frame_name}, Time {t_}: {bbox_data_str}\n")
